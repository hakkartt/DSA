...
package object warmup {
  ...
  def isRemovedByOne(s: String, t: String): (Boolean, Int) = {
    ...
    while (i < t.length) {
      if (s(i) != t(i)) {
        if (s.take(i) + s.substring(i+1) == t) return (true, i) else return (false, 0)
      }
      i += 1
    }
  ...
  def almostPalindrome(s: String): (Boolean, Option[Int]) = {
    ...
    while(i < s.length) {
      if (s(i) != s(s.length-1-i)) {
        if(isPalindrome(s.take(i) + s.substring(i+1))) return(true, Some(i))
        else if(isPalindrome( s.take(s.length-1-i) + s.substring(s.length-i))) return(true, Some(s.length-1-i))
        else return(false, None)
      }
      i += 1
    }
    return(true, Some(s.length-1))
  }
}
